...
function applyCurvedSideAnchorToLayout(layout, frame, distanceFromWall, width, sideOffset = STEP_TIER_OFFSET) {
  ...
  const visibleSide = layout.position === "right" ? "right" : layout.position === "left" ? "left" :
    (layout.placementPosition === "right" ? "right" : layout.placementPosition === "left" ? "left" : "center");
...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  const benchWallIsCurved = (forceBenchSeat || isFullWidthStep) && hasCurvedMainWallFace(layout, stepWidth);
  ...
  if (canUseBoundaryClip) {
    const boundaryLayout = shape === "radius" && curveStepToWall
      ? { ...layout, radiusCornerMode: "center" }
      : layout;
