...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  if (shape === "radius") {
    ...
    // Radius Corner is not a scaled rounded rectangle. It is a true 300 mm
    // front nosing plus a straight rear extension. When run grows, only x0
    // moves backwards; the 300 mm front radius stays unchanged.
    const TRUE_RADIUS = 0.3;
    const maxRadius = pos === "center" ? width * 0.5 : width;
    const radius = THREE.MathUtils.clamp(TRUE_RADIUS, 0.03, Math.max(0.03, maxRadius));

    const footprintRun = Math.max(run, radius);
    const x1 = footprintRun * 0.5;
    const x0 = -footprintRun * 0.5;
    const y0 = -width * 0.5;
    const y1 = width * 0.5;
    const segments = 24;
    const points = [];
    ...
    if (pos === "left") {
      ...
      addArc(x1 - radius, y1 - radius, radius, Math.PI * 0.5, 0);
      ...
      addArc(x1 - radius, y0 + radius, radius, 0, -Math.PI * 0.5);
    } else {
      points.push(new THREE.Vector2(x0, y0));
      points.push(new THREE.Vector2(x0, y1));
      points.push(new THREE.Vector2(x1 - radius, y1));
