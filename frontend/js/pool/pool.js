...
/* =====================================================
    ...
    const getLockedFloorDepthAt = (worldX, worldY) => {
      ...
      const baseLayout = wantsBenchSeat && s === 1 ? fullStepLayout : narrowLayout;
        ...
        const stepParamSource = liveParams ?? params;
        ...
        if (isCenteredCircular) {
          ...
          const previousLowerRuns = Array.from({ length: Math.max(0, s - 2) }, (_, i) => i + 2)
            .reduce((sum, stepIndex) => {
              const override = getBenchStepRunOverride(stepParamSource, stepIndex);
              const fallback = (stepParamSource?.stepShape === "circular" && stepParamSource?.stepPosition === "center")
                ? centeredCircularRun
                : ((stepParamSource?.stepShape === "diagonal" || stepParamSource?.stepShape === "circular") && layout.position !== "center"
                    ? diagonalStepSize
                    : STEP_LENGTH);
