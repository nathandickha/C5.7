...
// --------------------------------------------------------------
  ...
  _getStepNosingHit(step, worldPoint) {
    ...
    const edgeTol = Math.min(0.12, Math.max(0.045, run * 0.18));
    const sideTol = Math.min(0.08, Math.max(0.025, width * 0.08));

    const nearFront = Math.abs(local.x - bbox.max.x) <= edgeTol;
    const withinSpan = local.y >= bbox.min.y - sideTol && local.y <= bbox.max.y + sideTol;
    if (!nearFront || !withinSpan) return null;

    const rot = Number(step.rotation?.z) || 0;
    const axis = new THREE.Vector3(Math.cos(rot), Math.sin(rot), 0).normalize();
    ...
    const cursor = wall === "north" || wall === "south" ? "ns-resize" : "ew-resize";

    return { step, local, axis, cursor };
  }
  ...
  _startStepNosingPushPull(event, nosingHit) {
    ...
    this.captureUndoState?.("Push/pull step nosing");
    ...
    this.stepNosingDrag = {
      ...
      startValue: Number.isFinite(startValue) ? startValue : Number(step.userData?.stepRun) || 0.3,
      minValue: Number.isFinite(minValue) ? minValue : 0,
      maxValue: Number.isFinite(maxValue) && maxValue > 0 ? maxValue : 2,
      previousControlsEnabled
    };

    dom && (dom.style.cursor = nosingHit.cursor || "ew-resize");
