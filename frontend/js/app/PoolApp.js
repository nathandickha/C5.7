...
// --------------------------------------------------------------
  ...
  async _finishLShapeStepWallDrag(event) {
    ...
    this.captureUndoState?.("Move L-shape entry steps");
  ...
  setupStepSelection() {
    ...
    // L-shape entry steps can be click-held and dragged to any valid L perimeter wall.
    // The selected wall is resolved on release to avoid heavy rebuilds while dragging.
    dom.addEventListener("pointerdown", (event) => {
      if (event.button !== 0) return;
      if (!this.poolGroup || this.customizeMode || this.poolParams?.shape !== "L") return;
    ...
    dom.addEventListener("pointermove", (event) => {
      const drag = this.stepWallDrag;
      if (!drag || event.pointerId !== drag.pointerId || this.poolParams?.shape !== "L") return;
      ...
      const point = this._screenToPlanePoint(event.clientX, event.clientY, 0);
      const target = point ? this._getNearestLShapeStepWallFromPoint(point) : null;
      if (target) drag.target = target;
    }, true);
    ...
    // For L-shape pools, show a hand only while hovering entry steps because
    // those are the only targets that temporarily disable OrbitControls.
    dom.addEventListener("pointermove", (event) => {
      ...
      const step = hit[0].object;
      dom.style.cursor = this.poolParams?.shape === "L" ? "grab" : "pointer";
