  ...
  _applyStepDirectPushPullValue(mode, value, drag) {
    ...
    if (mode === "width") {
      ...
      if (this.poolParams?.stepShape !== "radius" && this.isCenteredCircularStepShape?.()) {
        ...
        // Bench Seat mode must be per-tier. Do not change the global
        // stepExtension when dragging one radius/rectangle tread, otherwise all
        // narrow treads inherit the same run and the selected step appears not
