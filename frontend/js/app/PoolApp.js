...
const concreteHatchTex = makeConcreteHatchTexture();
  ...
  focusCameraLikeStarterPreview(preset = null, { animate = false } = {}) {
    ...
    // Axonometric-style opening view: keep the same direction as the starter
    // cards, but use a very narrow perspective lens and a much longer camera
    // distance so the model reads like a parallel/orthographic axonometric
    // projection when the pool first opens.
    ...
    const halfVFov = THREE.MathUtils.degToRad((this.camera.fov || 10) * 0.5);
    const distance = Math.max(maxSize * 0.92, (maxSize * 0.58) / Math.max(Math.tan(halfVFov), 0.01));
...
// --------------------------------------------------------------
  ...
  _getStepNosingHit(step, worldPoint) {
    ...
    // Front nosing must have priority. Width handles are deliberately kept
    // away from the rounded front/nose zone so pulling the nose cannot widen
    // a radius/circular step by mistake.
    const frontTol = Math.min(0.14, Math.max(0.055, run * 0.16));
    const sideTol = Math.min(0.10, Math.max(0.035, width * 0.07));
    const frontExclusion = Math.max(0.32, frontTol + 0.22); // keeps side handles behind 300 mm nose

    const withinWidthSpan = local.y >= bbox.min.y - sideTol && local.y <= bbox.max.y + sideTol;
    const nearFront = Math.abs(local.x - bbox.max.x) <= frontTol && withinWidthSpan;
    ...
    // Side edges control width only from the straight side section, not the
    // rounded/nosing front section.
    const withinStraightSideRun =
      local.x >= bbox.min.x - sideTol &&
      local.x <= bbox.max.x - frontExclusion;

    if (withinStraightSideRun && Math.abs(local.y - bbox.max.y) <= sideTol) {
      return { step, local, axis: widthAxisPositive, cursor: widthCursor, mode: "width" };
    }

    if (withinStraightSideRun && Math.abs(local.y - bbox.min.y) <= sideTol) {
  ...
  _getStepMeshRun(step, fallback = 0.3) {
    if (!step?.geometry) return fallback;
    if (!step.geometry.boundingBox) step.geometry.computeBoundingBox();
    const bb = step.geometry.boundingBox;
    const baseLen = bb.max.x - bb.min.x;
    const scaledLen = baseLen * (step.scale?.x ?? 1);
    const savedRun = Number(step.userData?.stepRun);
    // For radius/capped steps the visible mesh length can be only the fixed
    // 300 mm nose while userData.stepRun stores the full logical run. Use the
    // logical run first so cascade deltas are calculated from the saved rule.
    if (Number.isFinite(savedRun) && savedRun > 0) return savedRun;
    if (Number.isFinite(scaledLen) && scaledLen > 0) return scaledLen;
    return fallback;
  }

  _selectReplacementStepByIndex(stepIndex, updateHighlight = true) {
    if (!this.poolGroup || !Number.isFinite(Number(stepIndex))) return null;
    let replacementStep = null;
    this.poolGroup.traverse((o) => {
      if (
        !replacementStep &&
        o?.userData?.isStep &&
        !o.userData?.isStepAddon &&
        Number(o.userData?.stepIndex) === Number(stepIndex)
      ) {
        replacementStep = o;
      }
    });
  ...
  _applyBenchCascadeRunOverride(selectedIndex, selectedRun) {
    if (!this.poolParams || !this.poolGroup) return false;

    const sortedSteps = this._getSortedPrimaryStepMeshes?.() || [];
    if (!sortedSteps.length) return false;

    const selectedRank = sortedSteps.findIndex((step) => Number(step.userData?.stepIndex) === Number(selectedIndex));
    if (selectedRank < 0) return false;

    const runs = { ...(this.poolParams.benchStepRuns || {}) };
    const selectedStep = sortedSteps[selectedRank];
    const oldSelectedRun = this._getStepMeshRun(selectedStep, Number(this.poolParams.stepExtension) || 0.3);
    const nextSelectedRun = Math.max(0.05, Number(selectedRun) || oldSelectedRun || 0.3);
    const delta = nextSelectedRun - oldSelectedRun;

    const setRun = (step, run) => {
      const idx = Number(step?.userData?.stepIndex);
      if (!Number.isFinite(idx)) return;
      if (idx === 1) {
        this.poolParams.bench2Extension = Math.max(0.3, run);
      } else {
        runs[String(idx)] = Math.max(0.05, run);
      }
    };

    const targetRunsByRank = new Map();
    sortedSteps.forEach((step, rank) => {
      ...
      const currentRun = this._getStepMeshRun(step, idx === 1 ? this.getBench2ExtensionValue?.() ?? 0.6 : Number(this.poolParams.stepExtension) || 0.3);
      let targetRun = currentRun;

      if (rank === selectedRank) {
        targetRun = nextSelectedRun;
      } else if (rank > selectedRank && idx <= 1) {
        // The full-width bench is wall-backed. When an upper wall-backed tread
        // is pulled forward, the bench must grow with it so the tread remains
        // contained by the bench after rebuild. Use the already-targeted upper
        // run, not the stale pre-rebuild run.
        const previousRun = targetRunsByRank.has(rank - 1)
          ? targetRunsByRank.get(rank - 1)
          : (sortedSteps[rank - 1] ? this._getStepMeshRun(sortedSteps[rank - 1], targetRun) : targetRun);
        targetRun = Math.max(currentRun + delta, previousRun, 0.3);
      }

      targetRunsByRank.set(rank, targetRun);
      setRun(step, targetRun);
    });
  ...
  _applyStepDirectPushPullValue(mode, value, drag) {
    ...
    if (mode === "width") {
      ...
      if (this.poolParams?.stepShape !== "radius" && this.isEqualCornerStepShape?.()) {
      ...
      } else if (this.poolParams?.stepShape !== "radius" && this.isCenteredCircularStepShape?.()) {
      ...
      // Radius Corner uses a fixed 300 mm nose; run cannot be smaller than that.
      ...
      if (this.poolParams?.stepShape !== "radius" && this.isCenteredCircularStepShape?.()) {
      ...
      } else if (this.poolParams?.stepShape !== "radius" && this.isEqualCornerStepShape?.() && selectedIndex !== 1) {
      ...
      } else if (stepBenchMode === "bench" && selectedIndex === 1) {
        const bench = THREE.MathUtils.clamp(nextRun, 0.3, 1.5);
        this._applyBenchCascadeRunOverride?.(selectedIndex, bench);
        this.poolParams.bench2Extension = bench;
        this._setStepSliderValue("stepExtension", bench);
        ...
        // Bench Seat mode must save the same cascade that the live slider uses.
        // Saving only the selected tread run makes the next rebuild detach the
        // full-width bench/lower steps from the selected tread.
        this._applyBenchCascadeRunOverride?.(selectedIndex, nextRun);
        this._setStepSliderValue("stepExtension", nextRun);
      } else {
    ...
    // Do not schedule rebuilds here. Push/pull commits call rebuild once on release.
  }


  _clearStepNosingPreview() {
    if (this.stepNosingPreviewGroup?.parent) {
      this.stepNosingPreviewGroup.parent.remove(this.stepNosingPreviewGroup);
    }
    this.stepNosingPreviewGroup = null;
  }

  _ensureStepNosingPreview(drag) {
    if (!drag || this.stepNosingPreviewGroup || !this.poolGroup) return;

    const group = new THREE.Group();
    group.name = "step-nosing-pushpull-preview";
    drag.previewItems = [];

    const selectedIndex = Number(drag.stepIndex);
    const sourceSteps = [];
    this.poolGroup.traverse((o) => {
      if (!o?.userData?.isStep) return;
      if (Number(o.userData?.stepIndex) !== selectedIndex) return;
      // Include radius extension add-ons for preview, but keep unrelated steps out.
      sourceSteps.push(o);
    });
    ...
    this.stepNosingPreviewGroup = group;
    this.poolGroup.add(group);
  }

  _previewStepNosingPushPull(mode, value, drag) {
    if (!drag || !this.poolGroup) return;
    this._ensureStepNosingPreview(drag);
    if (!drag.previewItems?.length) return;

    const delta = Number(value) - Number(drag.startValue);
    if (!Number.isFinite(delta)) return;

    // Lightweight drag feedback only. Do not rebuild geometry here.
    // Run drag moves the selected step/add-on along the run axis.
    // Width drag scales the selected step/add-on across its local width.
    for (const item of drag.previewItems) {
      const p = item.preview;
      if (!p) continue;

      p.position.copy(item.source.position);
      p.rotation.copy(item.source.rotation);
      p.scale.copy(item.source.scale);

      if (mode === "run") {
        const axis = drag.axis?.clone?.() || new THREE.Vector3(1, 0, 0);
        p.position.add(axis.multiplyScalar(delta));
      } else if (mode === "width") {
        const start = Math.max(0.05, Number(drag.startValue) || 0.05);
        const next = Math.max(0.05, Number(value) || start);
        const scale = next / start;
        p.scale.y *= scale;
      }
    }
  }
  ...
  _startStepNosingPushPull(event, nosingHit) {
    ...
    this.stepNosingDrag = {
      ...
      pendingValue: mode === "width" ? startWidth : startRun,
  ...
  async _finishStepNosingPushPull(event) {
    ...
    const pending = Number(drag.pendingValue);
    const start = Number(drag.startValue);
    const shouldCommit = Number.isFinite(pending) && Number.isFinite(start) && Math.abs(pending - start) > 0.005;

    this._clearStepNosingPreview?.();

    if (shouldCommit) {
      this._applyStepDirectPushPullValue?.(drag.mode, pending, drag);
      await this.rebuildPoolForCurrentShape?.();
      this._selectReplacementStepByIndex?.(drag.stepIndex, true);
      if (this._live) {
  ...
  _updateStepNosingPushPull(event) {
    ...
    drag.pendingValue = next;
    this._previewStepNosingPushPull?.(drag.mode, next, drag);
  }
  ...
  setupStepSelection() {
    ...
    const dom = this.renderer.domElement;
    if (this.__stepSelectionSetupDone) return;
    this.__stepSelectionSetupDone = true;
    ...
    dom.addEventListener("pointermove", (event) => {
      ...
      dom.style.cursor = nosingHover?.cursor || "grab";
  ...
  setupStepLayoutControls() {
    ...
    document.querySelectorAll("[data-step-wall]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        this.poolParams.benchStepRuns = {};
        this.poolParams.stepsOnlyStepRuns = {};
  ...
  setupStepExtensionSlider() {
    ...
    slider.addEventListener("input", () => {
      ...
      if (this.getStepBenchMode?.() === "bench") {
        ...
        let chainX = wallX;
        const benchRuns = { ...(this.poolParams.benchStepRuns || {}) };
        sortedSteps.forEach((step, rank) => {
          const stepIndex = Number.isFinite(Number(step.userData?.stepIndex))
            ? Number(step.userData.stepIndex)
            : rank;
          ...
          if (stepIndex === 1) {
            this.poolParams.bench2Extension = Math.max(0.3, targetLength);
          } else if (Number.isFinite(stepIndex)) {
            benchRuns[String(stepIndex)] = Math.max(0.05, targetLength);
          }
          this.updateScaledBoxTilingUVs(step);
        });

        this.poolParams.benchStepRuns = benchRuns;
  ...
  _normalizeStepParamsForBuild() {
    if (!this.poolParams) return;

    // Radius Corner is independent: width controls width, run controls extension.
    // It must not inherit diagonal/circular equal-footprint values on first load.
    if (this.poolParams.stepShape === "radius") {
      const width = Number(this.poolParams.stepWidth);
      if (!Number.isFinite(width) || width <= 0) this.poolParams.stepWidth = 0.9;

      const ext = Number(this.poolParams.stepExtension);
      if (!Number.isFinite(ext) || ext < 0.3) this.poolParams.stepExtension = 0.3;

      // Keep diagonalStepSize out of radius behaviour. Some old presets/slider
      // sync paths use it, but radius must always use a fixed 300 mm nose.
      this.poolParams.diagonalStepSize = 0.3;
    }

    if (!this.poolParams.stepBenchMode) this.poolParams.stepBenchMode = "bench";
    if (!["west", "east", "north", "south"].includes(this.poolParams.stepWall)) {
      this.poolParams.stepWall = "west";
    }
    if (!["left", "center", "right"].includes(this.poolParams.stepPosition)) {
      this.poolParams.stepPosition = "center";
    }
  }
  ...
  async rebuildPoolForCurrentShape() {
    this._normalizeStepParamsForBuild?.();
  ...
  setupShapeDropdown() {
    ...
    select.addEventListener("change", async (e) => {
      ...
      if (selectedStepIndex !== null) {
        this._selectReplacementStepByIndex?.(selectedStepIndex, true);
      } else {
        this.selectedStep = null;
        if (this.selectedHighlightMesh) this.selectedHighlightMesh.visible = false;
      }
